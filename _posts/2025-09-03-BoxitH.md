...
> Positional arguments must be widgets and are added to the `BoxitH` instance as children during creation.

> Example:

```py
sk.BoxitH(sk.Label('a'), sk.Label('b'), sk.Label('c'), )
```
...
```py
sk.BoxitH(size = "y35")
sk.BoxitH(size = "x120y35")
sk.BoxitH(size = "xchildreny40")
sk.BoxitH(size = "xchildrenychildren")
sk.BoxitH(size = "xchild_maxy40")
sk.BoxitH(size = "60")
sk.BoxitH(size = (120,35), size_hint = (None, None))
```
...
```py
sk.BoxitH(enable_events = True, on_event = 'width')
sk.BoxitH(enable_events = True, on_event = 'on_touch_down')
sk.BoxitH(enable_events = True, on_event = ['width','height','pos'])
sk.BoxitH(enable_events = True, on_event = {"size": lambda ins,v: print("size =",v)})
