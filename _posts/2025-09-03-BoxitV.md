...
> Positional arguments must be widgets and are added to the `BoxitV` instance as children during creation.

> Example:

```py
sk.BoxitV(sk.Label('a'), sk.Label('b'), sk.Label('c'), )
```
...
```py
sk.BoxitV(size = "y35")
sk.BoxitV(size = "x120y35")
sk.BoxitV(size = "xchildreny40")
sk.BoxitV(size = "xchildrenychildren")
sk.BoxitV(size = "xchild_maxy40")
sk.BoxitV(size = "60")
sk.BoxitV(size = (120,35), size_hint = (None, None))
```
...
```py
sk.BoxitV(enable_events = True, on_event = 'width')
sk.BoxitV(enable_events = True, on_event = 'on_touch_down')
sk.BoxitV(enable_events = True, on_event = ['width','height','pos'])
sk.BoxitV(enable_events = True, on_event = {"size": lambda ins,v: print("size =",v)})
